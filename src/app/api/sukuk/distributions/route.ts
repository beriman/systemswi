  ...
  try {
    ...
    if (productId) {
      distributions = db.prepare(`
        ...
        WHERE spd.product_id = ?
        ORDER BY spd.periode DESC
...
export async function POST(request: NextRequest) {
  try {
    ...
    const nisbahInvestor = Number(body.nisbah_investor);
    const nisbahPengelola = Number(body.nisbah_pengelola);

    if (!productId || !periode) {
      return NextResponse.json(
        { error: "product_id dan periode wajib diisi" },
        { status: 400 }
      );
    }
    ...
    if (totalProfit <= 0) {
      return NextResponse.json(
        { error: "Total profit harus lebih dari 0" },
        { status: 400 }
      );
    }

    // Get product for nisbah defaults
    const product = db.prepare("SELECT * FROM sukuk_products WHERE id = ?").get(productId);
    if (!product) {
      return NextResponse.json({ error: "Produk tidak ditemukan" }, { status: 404 });
    }

    const nInvestor = nisbahInvestor || product.nisbah_investor;
    const nPengelola = nisbahPengelola || product.nisbah_pengelola;

    const jumlahDibagikan = Math.round(totalProfit * (nInvestor / 100));

    // Get total active units
    const unitResult = db.prepare(
      "SELECT COALESCE(SUM(jumlah_unit), 0) as total FROM sukuk_investments WHERE product_id = ? AND status = 'aktif'"
    ).get(productId);
