...
import { getPublicEvents, type UpcomingEvent } from "@/lib/public";
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card";
import { Button } from "@/components/ui/button";

// API event shape (subset we need)
type ApiEvent = {
  id: string;
  name: string;
  startDate: string;
...
function apiEventToUpcoming(e: ApiEvent): UpcomingEvent {
  return {
    id: e.id,
    name: e.name,
    date: e.startDate || e.endDate || "",
    location: [e.location, e.venue].filter(Boolean).join(", "),
...
export default function EventsPage() {
  const [apiEvents, setApiEvents] = useState<UpcomingEvent[]>([]);
      ...
      try {
        ...
        if (!cancelled && Array.isArray(data.events) && data.events.length > 0) {
          const mapped = data.events
            .filter((e: ApiEvent) => e.status !== "cancelled")
            .map(apiEventToUpcoming);
          setApiEvents(mapped);
          setError(null);
        } else if (!cancelled) {
          // No events in API, fallback to mock
          setApiEvents(getPublicEvents());
          setError(null);
        }
      } catch (err) {
        if (!cancelled) {
          ...
          setApiEvents(getPublicEvents());
          setError("Menampilkan data lokal. Data live dari Google Sheets belum tersedia.");
        }
      } finally {
        ...
        <div className="container mx-auto px-4">
          ...
          {isLoading ? (
            <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3 max-w-6xl mx-auto">
              {[1, 2, 3].map((i) => (
                <Card key={i} className="overflow-hidden flex flex-col h-80">
            ...
            <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3 max-w-6xl mx-auto">
              {events.map((event) => {
                ...
                const dateValid = !isNaN(eventDate.getTime());
                  ...
                  <Card key={event.id} className="overflow-hidden flex flex-col hover:shadow-elegant transition-smooth group">
                    ...
                    <CardHeader>
                      <div className="flex items-center gap-2 text-sm text-muted-foreground mb-2">
                        <span>📅 {dateValid ? eventDate.toLocaleDateString("id-ID", {
                          day: "numeric",
                          month: "long",
                          year: "numeric",
                        }) : "Tanggal TBA"}</span>
